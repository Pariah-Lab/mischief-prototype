...
public class Boid : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.localPosition = Vector3.Lerp(transform.localPosition, position.FlattenedXZ(), Time.deltaTime * 5f);

        // update the boid in the direction of movement
        Debug.DrawRay(transform.position, velocity * 5f, Color.yellow, 0.5f);
        if (velocity != Vector3.zero)
        {
            transform.forward = Vector3.Lerp(transform.forward, -(oldPosition - transform.position).FlattenedXZ(), Time.deltaTime * 5f);
            oldPosition = transform.position.FlattenedXZ();
    ...
    {
        this.flock = flock;
        position = (Random.insideUnitSphere.normalized * flock.spawnRadius).FlattenedXZ();
        //position = (Random.insideUnitCircle.normalized * flock.spawnRadius).FlattenedXZ();
        Debug.Log("Position - " + position);
        velocity = Random.insideUnitSphere.normalized.FlattenedXZ();
    ...
    public void UpdateSimulation ()
    {
        ...
        velocity = Vector3.ClampMagnitude(velocity, flock.maxVelocity).FlattenedXZ();
    ...
    Vector3 Seperation(ref Boid[] boids)
    {
        ...
        foreach (int i in neighborIndices)
        {
            ...
            Vector3 neighborDir = (position - boids[i].position).FlattenedXZ();
            float neighborDist = neighborDir.magnitude;
            neighborDir = Vector3.Normalize(neighborDir).FlattenedXZ();
        ...
        force = Vector3.ClampMagnitude(force, flock.maxSteeringForce).FlattenedXZ();
    ...
    Vector3 Alignment(ref Boid[] boids)
    {
        ...
        foreach (int i in neighborIndices)
        {
            force += boids[i].velocity.FlattenedXZ();
        }
